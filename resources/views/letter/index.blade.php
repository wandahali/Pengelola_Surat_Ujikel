                        ...
                        <div class="card-body">
                            ...
                            @if (Auth::check())
                                @if (Auth::user()->role == 'staff')
                                    <a href="{{ route('letter.export') }}" class="btn btn-success">Export</a>
                                    <a href="{{ route('letter.create') }}" class="btn btn-primary">Tambah Data</a>
                                @endif
                            @endif
                            <table class="table">
                                <thead>
                                    ...
                                    @foreach ($letters as $key => $letter)
                                        <tr>
                                            ...
                                            <td>
                                                @if (Auth::check())
                                                    @if (Auth::user()->role == 'staff')
                                                        <a href="{{ route('letter.edit', $letter['id']) }}"
                                                    ...
                                                    @if (App\Models\Result::where('letter_id', $letter->id)->exists())
                                                        <a href="#" class="btn btn-success">Lihat Hasil Rapat</a>
                                                    @else
                                            ...
                                            <div class="modal fade" id="deleteModal-{{ $letter->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form action="{{ route('letter.delete', $letter->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
                                                    <button type="submit"
